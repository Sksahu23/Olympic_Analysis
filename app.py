...
if user_menu == 'Overall Analysis':
    ...
    events_per_year = sk.pivot_table(index='Sport', columns='Year', values='Event', aggfunc='count').fillna(0).astype(
        'int')
    ax = sns.heatmap(events_per_year, annot=True, cmap='flare')
    st.pyplot(fig)
...
if user_menu == 'Country-wise Analysis':
    ...
    ax = sns.heatmap(pt, annot=True, cmap='flare')
    st.pyplot(fig)
...
if user_menu == 'Athlete wise Analysis':
    ...
    fig = px.line(final, x="Year", y=["Male", "Female"], color_discrete_map={'Male': 'blue', 'Female': 'red'})
    fig.update_layout(autosize=False, width=1000, height=600)
    st.plotly_chart(fig)
